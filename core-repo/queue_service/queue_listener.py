...
class QueueListener(metaclass=ABCMeta):
    def __init__(self, queue_service: IRedisQueueService,
                 queue_name: str,
    ...
    def listen(self):
        while True:
            message = self.queue_service.get_message_from_queue(
                self.queue_name)
            if message:
                message = self._process_message(message)
            if self.out_queue_name and message:
                self.queue_service.add_message_to_queue(
                    self.out_queue_name, message)
