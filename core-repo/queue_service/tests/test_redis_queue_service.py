...
def test_add_message_to_queue(
    ...
    new_queue_length = redis_queue_service.add_message_to_queue(queue_name, message)
    assert new_queue_length - queue_length == 1


def test_get_message_from_queue(
    queue_name: str, redis_queue_service: RedisQueueService, redis_instance: Redis
):
...
def test_get_message_from_queue_when_no_message(
    queue_name: str, redis_queue_service: RedisQueueService, redis_instance: Redis
):
...
def test_delete_message_from_queue(
    queue_name: str, redis_queue_service: RedisQueueService, redis_instance: Redis
):
    # generate messages
    messages = [DefaultMessage() for _ in range(10)]
    [redis_instance.rpush(queue_name, message.to_json()) for message in messages]
...
def test_delete_non_existent_message_from_queue(
    queue_name: str, redis_queue_service: RedisQueueService
):
    no_of_ele_removed = redis_queue_service.delete_message_from_queue(
