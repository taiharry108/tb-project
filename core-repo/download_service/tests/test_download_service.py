...
def download_service() -> DownloadService:
    return DownloadService(max_connections=5, max_keepalive_connections=5, headers={}, store_service=FSStoreService(), proxy={})
...
async def test_download_img(download_service: DownloadService, img_url: str):
    result = await download_service.download_img(img_url, download_path="test", filename="test.png")
...
async def test_download_imgs(download_service: DownloadService, async_service: AsyncService, download_path, img_url: str):
    n = 2
    img_list = [{"url": img_url, "filename": f"{idx}", "test_key": "test_val"} for idx in range(n)]
    
    counter = 0
    async for result_dict in download_service.download_imgs(async_service, Path(download_path), img_list, headers={"Referer": ""}):
        counter += 1
