...
@inject
async def get_manga_site_id(site: MangaSiteEnum, crud_service: CRUDService = Depends(
...
def get_scraping_service_from_site(
        site: MangaSiteEnum,
        ss_factory = Provider[Container.scraping_service_factory]) -> Union[MSSService, ASSService]:
    return ss_factory(
        site)
...
def get_scraping_service_from_manga(
        manga_site_name: str = Depends(get_manga_site_from_manga)) -> MSSService:
    return get_scraping_service_from_site(manga_site_name)


@inject
def get_scraping_service_from_anime(
        manga_site_name: str = Depends(get_manga_site_from_anime)) -> ASSService:
    return get_scraping_service_from_site(manga_site_name)


@inject
async def get_manga_from_chapter_id(db_chapter: Chapter = Depends(get_chapter_from_id),
                                     session: AsyncSession = Depends(get_db_session),) -> Manga:
    return await get_db_manga_from_id(db_chapter.manga_id, session)


@inject
async def get_anime_from_episode_id(db_episode: Episode = Depends(get_episode_from_id),
...
async def get_scraping_service_from_episode(
        ...
        session: AsyncSession = Depends(get_db_session),) -> ASSService:
