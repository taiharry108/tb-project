...
class CopyMangaScrapingService(MangaSiteScrapingService):
    ...
    async def search_manga(self, keyword: str) -> List[Manga]:
        ...
        result = [
            Manga(url=construct_url(item["path_word"]), name=item["name"])
            for item in result
        ]
        return result
    ...
    async def extract_meta_from_soup(
        self, soup: BeautifulSoup, manga_url: str
    ) -> Dict[str, any]:
    ...
    async def get_chapters(
        ...
        type_dict = {
            t_dict["id"]: t_dict["name"] for t_dict in json_data["build"]["type"]
        }
