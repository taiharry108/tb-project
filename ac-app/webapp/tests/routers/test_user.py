...
from database.models import MangaSite, Manga, Chapter, Page, History, User, Anime, Episode, AHistory
...
@pytest.fixture(scope="module")
def a_history_path(): return "/user/a_history"
...
@pytest.fixture(scope="module")
async def anime_name(): return "test_anime"


@pytest.fixture(scope="module")
async def episode_title(): return "test_episode"


@pytest.fixture(scope="module")
async def episode_key(): return "key"


@pytest.fixture(scope="module")
async def anime_url(): return "https://test_anime.com"
    ...
    async with database.session() as session:
        async with session.begin():
            ...
            session.add(Manga(name=manga_name,
                        url=manga_url))
            session.add(Anime(name=anime_name, url=anime_url,
                              sub="test_sub", year="test_year",
                              season="test_season", eps="test_eps"
                              ))
...
@pytest.fixture(scope="module")
async def anime_id(anime_url: str, crud_service: CRUDService, database: DatabaseService) -> int:
    async with database.session() as session:
        async with session.begin():
            db_anime = await crud_service.get_item_by_attr(session, Anime, "url", anime_url)
            return db_anime.id
...
@pytest.fixture(scope="module")
async def episode_id(episode_key: str, episode_title: str, anime_id: int,
                     crud_service: CRUDService,
                     database: DatabaseService) -> int:
    async with database.session() as session:
        async with session.begin():
            session.add(Episode(title=episode_title,
    ...
    async with database.session() as session:
        async with session.begin():
            db_episode = await crud_service.get_item_by_attr(session, Episode, "manual_key", episode_key)
...
async def test_add_a_history(
    ...
    history_animes = await crud_service.get_items_of_obj(db_session, User, user_id, "history_animes")
...
async def test_update_a_history(a_history_path: str,
                                client: AsyncClient,
                                anime_id: int,
                                user_id: int,
                                episode_id: int,
...
async def test_get_a_history(a_history_path: str, client: AsyncClient, anime_name: str, anime_url: str, episode_title: str):
    resp = await client.get(a_history_path)
...
async def test_del_a_history(a_history_path: str,
                             client: AsyncClient,
                             anime_id: int,
                             user_id: int,
                             crud_service: CRUDService,
