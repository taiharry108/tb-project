...
@pytest.fixture(scope="module")
def episodes_path(): return "/api/episodes"


@pytest.fixture(scope="module")
def episode_path(): return "/api/episode"
...
@pytest.fixture(scope="module")
def pages_path(): return "/api/pages"
...
async def run_before_and_after_tests(database: DatabaseService):
    ...
    async with database.session() as session:
        async with session.begin():
            ...
            session.add(MangaSite(name="anime1",
                        url="https://anime1.me/"))
...
async def test_search_anime(search_path: str, client: AsyncClient,
                            anime_name: str,
                            anime_url: str,
...
async def test_get_episodes(anime_url: str, episodes_path: str,
                            client: AsyncClient,
                            crud_service: CRUDService,
                            db_session: AsyncSession):

    anime_id = await crud_service.get_id_by_attr(db_session, Anime, "url", anime_url)
    resp = await client.get(episodes_path, params={
        "anime_id": anime_id
    })
...
async def test_get_episode(anime_url: str, episode_path: str,
                           client: AsyncClient,
                           crud_service: CRUDService,
                           db_session: AsyncSession):
    anime_id = await crud_service.get_id_by_attr(db_session, Anime, "url", anime_url)
    db_episodes = await crud_service.get_items_by_same_attr(db_session, Episode, "anime_id", anime_id)
    resp = await client.get(episode_path, params={
        "episode_id": db_episodes[0].id
    })
