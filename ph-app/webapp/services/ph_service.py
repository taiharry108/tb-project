...
def _get_videos_from_pc_vid_list(search_result_list: BeautifulSoup) -> list[Video]:
    def parse_search_result(result):
        ...
        return Video(
            ...
            duration=a_tag.find("div", class_="marker-overlays")
            .find("var", class_="duration")
            .text.strip(),
            username=result.find("div", class_="usernameWrap").text.strip(),
            upload_date=result.find("div", class_="videoDetailsBlock")
            .find("var", class_="added")
            .text.strip(),
        )
...
def download_video(filepath: Path, vid_url: str) -> None:
    ...
    if filepath.exists():
        return
    downloader = M3u8Downloader(vid_url, filepath, tempdir=tempdir, poolsize=10)
    downloader.start()
...
async def search_ph(
    keyword: str,
    page: int,
    search_url: str = Depends(Provide[Container.config.ph_service.search_url]),
    download_service: DownloadService = Depends(Provide[Container.download_service]),
    async_service: AsyncService = Depends(Provide[Container.async_service]),
    download_path: str = Depends(Provide[Container.config.api.download_path]),
) -> SearchResult:
    url = search_url.format(keyword.replace(" ", "+"), page)

    soup: BeautifulSoup = await download_service.get_soup(url)
    next_page = soup.find("li", class_="page_next").find("a")["href"].split("=")[-1]

    search_result_list = soup.find("ul", id="videoSearchResult")
    search_result_list = search_result_list.find_all("li", class_="pcVideoListItem")

    videos = _get_videos_from_pc_vid_list(search_result_list)

    async for result in download_thumbnails(
        videos, download_service, async_service, download_path + "/thumbnail"
    ):
...
def download_thumbnails(
    ...
    return download_service.download_imgs(
...
async def get_vid_result(
    vid_id: str,
    vid_url: str = Depends(Provide[Container.config.ph_service.vid_url]),
    download_service: DownloadService = Depends(Provide[Container.download_service]),
    download_path: str = Depends(Provide[Container.config.api.download_path]),
    async_service: AsyncService = Depends(Provide[Container.async_service]),
) -> VidResult:
    ...
    def get_vid_url(soup: BeautifulSoup) -> str:
        ...
        media_def = list(filter(lambda x: isinstance(x["quality"], str), media_def))
